...
- (void) testRawSignatures {
    ...
    NSData* sigObj = [privObj rawSign: message];
    XCTAssertNotNil(sigObj);
    XCTAssertEqual(sigObj.length, 64u);
    XCTAssert([pubObj verifyRawSignature: sigObj ofData: message]);
    XCTAssert([privObj.publicKey verifyRawSignature: sigObj ofData: message]);
...
- (void) testSignatures {
    ...
    NSData* signature = [alice rawSign: message];
    XCTAssertNotNil(signature);
    XCTAssertEqual(signature.length, 64u);
    NSLog(@"Signature = %@", signature);

    XCTAssert([alice.publicKey verifyRawSignature: signature ofData: message]);
}
